			...
			async doCommand(aAddon) {
				...
				if (gViewController.currentViewObj != gListView) {
					await new Promise(resolve => {
						document.addEventListener("ViewChanged", resolve, {
							once: true
						});
...
var amfSort = {
	init: function () {
		amfSort.header = document.getElementById("header-inner");
...
window.addEventListener("load", function (e) {
	...
	window.addEventListener('ViewChanged', function (e) {
		...
		/* 		if (e.target.getAttribute("type") === "extension"){
		window.history.go(-1 * counter -1);
		counter = 0;
		// let state = lastState;
 ...
 var pi = document.createProcessingInstruction('xml-stylesheet', 'href="chrome://addonmanagerfix/content/addonmanagerfix.css" type="text/css"');
